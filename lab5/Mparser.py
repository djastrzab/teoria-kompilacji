...
def p_statement(p):
    """statement : ifstatement
                ...
                | CONTINUE ';'"""
    if p[1] == "break":
        p[0] = AST.BreakInstruction(line_no=p.lineno(1))
        
    elif p[1] == "continue":
        p[0] = AST.ContinueInstruction(line_no=p.lineno(1))
    else:
        p[0] = p[1]
...
def p_return_statement(p):
    """returnstatement : RETURN 
                        | RETURN expr"""
    if len(p) == 2:
        p[0] = AST.ReturnStatement(p[1], line_no=p.lineno(1))
    else:
        p[0] = AST.ReturnStatement(p[1], p[2], line_no=p.lineno(1))
...
def p_assign_statement(p):
    ...
    p[0] = AST.BinExpr(p[2], p[1], p[3], line_no=p.lineno(2))
...
def p_assignable(p):
    ...
    if len(p) == 2:
        ...
        p[0] = AST.BinExpr(p[1] + "[,]", p[3], p[5], line_no=p.lineno(1))
...
def p_expr4(p):
    """expr : '-' expr %prec UNARY
            ...
            """
    if len(p)==5:
        p[0] = AST.MatWord(p[1], p[3], line_no=p.lineno(2))
    elif len(p) == 4:
        if p[1] == '(' or p[1] == '{':
            p[0] = p[2]
        elif p[1] == '[':
        ...
        if p[1] == '-':
            p[0] = AST.UnaryMinus(p[2], line_no=p.lineno(1))
        else:
            p[0] = AST.UnaryTranspose(p[1], line_no=p.lineno(2))
...
def p_if_statement(p):
    ...
    if len(p) > 6:
        p[0] = AST.IfElse(p[3], p[5], p[7], line_no=p.lineno(1))
    else:
        p[0] = AST.IfElse(p[3], p[5], line_no=p.lineno(1))
...
def p_for_loop(p):
    ...
    p[0] = AST.ForLoop(p[2], p[4], p[5], line_no=p.lineno(1))
...
def p_matrix_initializer1(p):
    """matrixinitializer : innerlist"""                
    p[0] = [AST.Vector(p[1])]

def p_matrix_initializer(p):
    """matrixinitializer : morelist"""
    p[0] = p[1]
